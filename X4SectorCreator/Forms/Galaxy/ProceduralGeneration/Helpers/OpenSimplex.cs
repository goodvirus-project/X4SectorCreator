...
namespace X4SectorCreator.Forms.Galaxy.ProceduralGeneration.Helpers
{
    public static class OpenSimplex2
    {
         ...
         * Custom
         */
        public static float[] GenerateNoiseMap(int mapWidth, int mapHeight, int seed, float scale, int octaves, float persistance, float lacunarity, Point offset)
        {
            ...
            {
                float offsetX = random.Next(-100000, 100000) + (float)offset.X;
                float offsetY = random.Next(-100000, 100000) + (float)offset.Y;
            ...
            for (int x = 0, y; x < mapWidth; x++)
            {
                for (y = 0; y < mapHeight; y++)
                {
                    ...
                    for (int i = 0; i < octaves; i++)
                    {
                        ...
                        float openSimplexValue = Noise2(seed, sampleX, sampleY) * 2 - 1;

                        noiseHeight += openSimplexValue * amplitude;
